...
class NormalizedMetrics:
    ...
    def normalize_db_metrics(self, df: pd.DataFrame, component_name: str = "KingbaseES") -> pd.DataFrame:
        ...
        for _, row in df.iterrows():
            ...
            # 资源利用率：优先使用实际监控数据，否则使用估算值
            avg_cpu_percent = row.get('avg_cpu_percent')
            avg_memory_percent = row.get('avg_memory_percent')
            
            if pd.notna(avg_cpu_percent) and avg_cpu_percent is not None:
                cpu_utilization = float(avg_cpu_percent)
            else:
                # 估算：CPU利用率 = (实际TPS / 理论最大TPS) * 100，理论值基于经验
            ...
            if pd.notna(avg_memory_percent) and avg_memory_percent is not None:
                memory_utilization = float(avg_memory_percent)
            else:
            ...
            results.append({
                ...
                'memory_utilization_pct': round(memory_utilization, 2) if memory_utilization is not None else None,
                'avg_cpu_percent': round(avg_cpu_percent, 2) if pd.notna(avg_cpu_percent) and avg_cpu_percent is not None else None,
                'max_cpu_percent': round(row.get('max_cpu_percent'), 2) if pd.notna(row.get('max_cpu_percent')) and row.get('max_cpu_percent') is not None else None,
                'avg_memory_percent': round(avg_memory_percent, 2) if pd.notna(avg_memory_percent) and avg_memory_percent is not None else None,
                'max_memory_percent': round(row.get('max_memory_percent'), 2) if pd.notna(row.get('max_memory_percent')) and row.get('max_memory_percent') is not None else None,
                'avg_memory_used_gb': round(row.get('avg_memory_used_gb'), 3) if pd.notna(row.get('avg_memory_used_gb')) and row.get('avg_memory_used_gb') is not None else None,
    ...
    def generate_capacity_extrapolation(self, normalized_df: pd.DataFrame, target_slo: Dict) -> pd.DataFrame:
        ...
        if target_slo.get('component_type') == 'DB':
            ...
            if len(valid_data) > 0:
                ...
                # 考虑实际CPU利用率：如果测试时CPU利用率较低，说明还有余量
                # 外推时假设CPU利用率会提高到合理水平（70-80%），但不超过实际测试值
                actual_cpu_util = best.get('cpu_utilization_pct', 80)  # 默认80%
                if pd.notna(best.get('avg_cpu_percent')):
                    actual_cpu_util = best['avg_cpu_percent']
                
                # 外推时假设的CPU利用率：如果测试时利用率较低，假设会提高到75%
                # 如果测试时利用率已经很高，使用实际值
                target_cpu_util = max(actual_cpu_util, 75.0) if actual_cpu_util < 75.0 else actual_cpu_util
                target_cpu_util = min(target_cpu_util, 85.0)  # 不超过85%，留有余量
                
                # 计算所需核心数：基于目标CPU利用率
                # tps_per_core 是在测试环境下的值，外推时考虑目标利用率
                effective_tps_per_core = best['tps_per_core'] * (target_cpu_util / 100.0) if target_cpu_util > 0 else best['tps_per_core']
                required_cores = int(np.ceil(target_tps / effective_tps_per_core))
                
                # 计算所需内存（基于tps_per_gb，考虑实际内存利用率）
                actual_mem_util = best.get('memory_utilization_pct', 70)  # 默认70%
                if pd.notna(best.get('avg_memory_percent')):
                    actual_mem_util = best['avg_memory_percent']
                
                # 外推时假设的内存利用率：类似CPU，假设会提高到合理水平
                target_mem_util = max(actual_mem_util, 70.0) if actual_mem_util < 70.0 else actual_mem_util
                target_mem_util = min(target_mem_util, 80.0)  # 不超过80%
                
                effective_tps_per_gb = best['tps_per_gb_memory'] * (target_mem_util / 100.0) if target_mem_util > 0 else best['tps_per_gb_memory']
                required_memory_gb = int(np.ceil(target_tps / effective_tps_per_gb))
                ...
                recommendations.append({
                    ...
                    'baseline_cpu_utilization_pct': round(actual_cpu_util, 2),
                    'baseline_memory_utilization_pct': round(actual_mem_util, 2) if actual_mem_util is not None else None,
                    'extrapolation_target_cpu_util_pct': round(target_cpu_util, 2),
                    'extrapolation_target_mem_util_pct': round(target_mem_util, 2) if target_mem_util is not None else None,
                })
            ...
            if len(valid_data) > 0:
                ...
                # 外推时假设的CPU利用率：如果测试时利用率较低，假设会提高到75%
                target_cpu_util = max(actual_cpu_util, 75.0) if actual_cpu_util < 75.0 else actual_cpu_util
                target_cpu_util = min(target_cpu_util, 85.0)  # 不超过85%
                
                # 计算所需核心数：基于目标CPU利用率
                effective_msg_per_sec_per_core = best['msg_per_sec_per_core'] * (target_cpu_util / 100.0) if target_cpu_util > 0 else best['msg_per_sec_per_core']
                required_cores = int(np.ceil(target_msg_per_sec / effective_msg_per_sec_per_core))
                ...
                effective_msg_per_sec_per_gb = best['msg_per_sec_per_gb_memory'] * (target_mem_util / 100.0) if target_mem_util > 0 else best['msg_per_sec_per_gb_memory']
                required_memory_gb = int(np.ceil(target_msg_per_sec / effective_msg_per_sec_per_gb))
