...
def batch_process(
    ...
    Returns:
        合并后的归一化指标DataFrame，如果无数据则返回None
    """
        ...
        try:
            ...
            if len(db_normalized) > 0:
                ...
                if pd.notna(best.get('avg_cpu_percent')):
                    print(f"  ✓ CPU使用率: 平均 {best['avg_cpu_percent']:.2f}%, 最大 {best.get('max_cpu_percent', 'N/A')}")
                if pd.notna(best.get('avg_memory_percent')):
                    print(f"  ✓ 内存使用率: 平均 {best['avg_memory_percent']:.2f}%, 最大 {best.get('max_memory_percent', 'N/A')}")
            else:
                print(f"  ⚠ 未找到有效数据")
    ...
    if all_normalized:
        ...
        for comp_type in combined['component_type'].unique():
            ...
            if comp_type == 'DB':
                ...
                # 显示实际资源使用率（如果有）
                if 'avg_cpu_percent' in comp_data.columns and comp_data['avg_cpu_percent'].notna().any():
                    print(f"\n资源使用率（实际监控）:")
                    print(f"  CPU利用率: 平均 {comp_data['avg_cpu_percent'].mean():.2f}%, "
                          f"最大 {comp_data['max_cpu_percent'].max():.2f}%")
                if 'avg_memory_percent' in comp_data.columns and comp_data['avg_memory_percent'].notna().any():
                    print(f"  内存利用率: 平均 {comp_data['avg_memory_percent'].mean():.2f}%, "
                          f"最大 {comp_data['max_memory_percent'].max():.2f}%")
                else:
                    print(f"\n资源利用率（估算）:")
...
def capacity_extrapolation_example(
    ...
    """
    容量外推示例：基于SLO反推所需资源（不保存文件，直接打印结果）
    
    Args:
        normalized_df: 归一化指标DataFrame
        target_slo: 目标SLO约束
    """
    print("\n=== 容量外推计算 ===")
...
def main():
    ...
    # 批量处理（不保存文件，直接返回归一化结果）
    normalized_df = batch_process(
    ...
    # 容量外推示例（使用内存中的归一化数据）
    if args.extrapolate and normalized_df is not None:
        if args.target_tps:
            target_slo = {
                'component_type': 'DB',
            ...
            capacity_extrapolation_example(normalized_df, target_slo)
